...
def handle_private(message,chatid,msgid):
		...
		smsg = bot.send_message(message.chat.id, '__Uploading__', reply_to_message_id=message.message_id)
		upsta = threading.Thread(target=lambda:upstatus(f'{message.chat.id}{message.message_id}upstatus.txt',smsg),daemon=True)
		upsta.start()
		
        post_data = {"api_key": DOODSTREAM_API_KEY}
        filename = file.split("/")[-1]
        post_files = {"file": (filename, open(file, "rb"))}
        res = requests.post(f"{base_url}upload/server?key={DOODSTREAM_API_KEY}", data=post_data, files=post_files).json()
        if res["msg"] == "OK":
            return res
        else:
            raise TypeError(
